package com.example.ellipseimage;
...
public class SelectableRoundedCornersImageView extends ImageView {
	...
	private void setCornerRadiusFromAttribute(Context context, AttributeSet attrs) {
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SelectableRoundedCornersImageView);
 		if (typedArray != null) {
			leftTopCornerRadius = typedArray.getDimension(
					R.styleable.SelectableRoundedCornersImageView_leftTopCornerRadius, 0f);
			rightTopCornerRadius = typedArray.getDimension(
					R.styleable.SelectableRoundedCornersImageView_rightTopCornerRadius, 0f);
			leftBottomCornerRadius = typedArray.getDimension(
					R.styleable.SelectableRoundedCornersImageView_leftBottomCornerRadius, 0f);
			rightBottomCornerRadius = typedArray.getDimension(
					R.styleable.SelectableRoundedCornersImageView_rightBottomCornerRadius, 0f);
		}
	}
	...
	private void initPath(int width, int height) {
		...
		float[] radius = { leftTopCornerRadius, leftTopCornerRadius, rightTopCornerRadius, rightTopCornerRadius,
